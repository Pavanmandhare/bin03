...
document.addEventListener("DOMContentLoaded", function () {
    document.getElementById("requestForm").addEventListener("submit", function (event) {
        ...
        const formData = {
            ...
            middle_name: formElements["middle_name"].value,
            last_name: formElements["last_name"].value,
            reason: formElements["reason"].value,
            h_name: formElements["h_name"].value,
            h_address: formElements["h_address"].value,
            state: formElements["state"].value,
            district: formElements["district"].value,
            city: formElements["city"].value,
            pin_code: formElements["pin_code"].value,
            a_first_name: formElements["a_first_name"].value,
            a_middle_name: formElements["a_middle_name"].value,
            a_last_name: formElements["a_last_name"].value,
            relation: formElements["relation"].value,
            mobile: formElements["mobile"].value,
            alternate: formElements["alternate"].value,
            blood_group: formElements["blood_group"].value,
            donors_required: formElements["donors_required"].value,
            blood_required_date: formElements["blood_required_date"].value,
            r_first_name: formElements["r_first_name"].value,
            r_middle_name: formElements["r_middle_name"].value,
            r_last_name: formElements["r_last_name"].value,
        };

        // Fetch the next sequential user ID from Google Apps Script
        fetch("https://script.google.com/macros/s/AKfycbwvirfz4N0PzhA4ed78fUtzay7T_dBKeVzwmLqWha2km9K0mZQXtMkjPRy-ulfzJlNG9Q/exec")
...
function showSuccessPopup(userId) {
    const popup = document.createElement("div");
    popup.innerHTML = `<div style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: #333; color: white; padding: 20px; border-radius: 10px; text-align: center; z-index: 1000;">
        <p>Form submitted successfully!</p>
        <p>Your Unique ID: <strong>${userId}</strong></p>
        <button id="closePopup" style="margin-top: 10px; padding: 5px 10px; background: #ff5e5e; color: white; border: none; cursor: pointer;">OK</button>
    </div>`;
...
/*document.getElementById("requestForm").addEventListener("submit", async function (event) {
    ...
    try {
        let response = await fetch("https://script.google.com/macros/s/AKfycbzkx-8aRoo5xX9qjToRCmhzVioR35gYTsv9KfHBRFzhBOL5v-9behGAL6K3IWPnpAcLJw/exec", {
            method: "POST",
            mode: "no-cors",
            headers: { "Content-Type": "application/json" },
